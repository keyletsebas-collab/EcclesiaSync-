...
public partial class MainPage : ContentPage
{
    ...
    public MainPage()
    {
        ...
        {
            if (e.Url?.StartsWith("hybrid:") is true)
            {
                ...
                try
                {
                    ...
                    var queryString = e.Url[(e.Url.IndexOf('?') + 1)..];
                    var title = "";
                    var body = "";
    ...
    private async Task InitializeAppAsync()
    {
        try
        {
            ...
            const string supabaseUrl = "https://hkmmotgmfsfdxyavsozx.supabase.co";
            const string supabaseKey = "sb_publishable_Mog0DO6L05Zt6sxaeExArw_J0HZ3f6L";
...
#pragma warning disable CA1416 // Platform compatibility - guarded by Build.VERSION.SdkInt checks
    private static async void ShowLocalNotification(string title, string body)
    {
#if ANDROID
        try
        {
            ...
            const string channelId = "verbumsync_channel";
            ...
            int iconId = context.Resources?.GetIdentifier("appicon", "mipmap", context.PackageName) ?? 0;
            if (iconId == 0)
            {
                iconId = context.Resources?.GetIdentifier("appicon", "drawable", context.PackageName) ?? 0;
            }
            if (iconId == 0)
            {
